...
void print(const char* text, uint32_t x, uint32_t y, uint16_t attr) {
  ca(*p)[VIDEO_COLS] = (ca(*)[VIDEO_COLS])VIDEO;  // p apunta a filas y cada fila tiene 80 celdas que arranca en VIDEO(0x000B8000) y cada celda es ca
  int32_t i;
  for (i = 0; text[i] != 0; i++) {
...
void screen_draw_layout(void) {
  ...
  print("Matafuegos",10, 10, C_BG_BLUE | C_FG_WHITE); // llamo a la funcion print de arriba para poner el nombre del grupo 
